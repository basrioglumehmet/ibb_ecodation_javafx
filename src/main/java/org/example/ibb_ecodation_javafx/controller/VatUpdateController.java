...
public class VatUpdateController {
    ...
    private void updateData() {
        ...
        if (validator.runValidatorEngine().isEmpty()) {
            try {
                ...
                LocalTime timePart = selectedItemData.getTransactionDate() != null
                        ? selectedItemData.getTransactionDate().toLocalDateTime().toLocalTime()
                        : LocalTime.MIDNIGHT;

                LocalDateTime combinedDateTime = LocalDateTime.of(transactionDate, timePart);

                // Timestamp dönüşümü
                Timestamp timestamp = Timestamp.valueOf(combinedDateTime);

                selectedItemData.setTransactionDate(timestamp);
        ...
        //, vat -> {
                ...
                //                    closeVatDialog();
                //                }
                vatService.update(selectedItemData);
                resultLabel.setText(languageService.translate("label.vatUpdated"));
                closeVatDialog();
            } catch (Exception e) {
